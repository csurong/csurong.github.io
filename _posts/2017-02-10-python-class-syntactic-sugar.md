...
我们已然知道init的作用：在实例化一个类的时候，会默认自动调用这个方法执行。这就是说，我们不必明显的告诉Python去执行这个方法，在运行下面这行代码时，Python已经悄悄自己执行这个方法了：`lili = Girlfriend('乌黑长发')`
...
## 参考
* [原文](https://zhuanlan.zhihu.com/p/22810357)